...
const REST_VELOCITY = new Vec3(0.000001, 0, 0);
...
class FirstPersonController extends Script {
  ...
  _move(dt) {
    ...
    if (velocity.length() < REST_VELOCITY.length()) {
      velocity.set(0, 0, 0);
    }
  }
  ...
  update(dt) {
    ...
    if (!this.entity.collision || !this.entity.rigidbody) {
      console.log("no collision or rigidbody");
      return;
    }

    console.log(
      "this.entity.rigidbody.angularFactor",
      this.entity.rigidbody.angularFactor
    );
